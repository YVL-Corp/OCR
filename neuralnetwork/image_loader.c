...
int load_images_from_folder(const char *folder_path, int label, ImageData **images, int *current_count, int max_images) {
    ...
    int total_files = valid_files->len;
    if (total_files == 0) {
    ...
    // Charger les images
    int loaded = 0;
    int max_to_load = (total_files < max_images) ? total_files : max_images;
...
int load_dataset(const char *root_path, ImageData **images, int max_per_letter) {
    ...
    *images = malloc(26 * max_per_letter * sizeof(ImageData));
    ...
    while ((entry_name = g_dir_read_name(dir)) != NULL) {
        ...
        if (label < 0 || label >= 26) {
            printf("Skipping folder '%s' (invalid label)\n", entry_name);
            g_free(letter_path);
            continue;
        }
...
static gboolean parse_grid_filename(const char *filename, int *x, int *y) {
    // Format attendu: XX_YY.bmp (ex: 0_0.bmp, 12_5.bmp, 100_200.bmp)
    int len = strlen(filename);
    
    // Vérifier l'extension .bmp
    if (len < 7 || strcmp(filename + len - 4, ".bmp") != 0) {
        return FALSE;
    }
    
    // Trouver le underscore
    const char *underscore = strchr(filename, '_');
    if (!underscore) return FALSE;
    
    // Vérifier qu'il y a au moins un chiffre avant et après le underscore
    if (underscore == filename || underscore >= filename + len - 4) {
        return FALSE;
    }
    ...
    if (x_len >= 16 || y_len >= 16 || x_len < 1 || y_len < 1) {
        return FALSE;
    }
    
    strncpy(x_str, filename, x_len);
    x_str[x_len] = '\0';
    
    strncpy(y_str, underscore + 1, y_len);
    y_str[y_len] = '\0';
    
    // Vérifier que ce sont bien des nombres
    for (int i = 0; i < x_len; i++) {
        if (x_str[i] < '0' || x_str[i] > '9') return FALSE;
    }
    for (int i = 0; i < y_len; i++) {
        if (y_str[i] < '0' || y_str[i] > '9') return FALSE;
    }
...
int load_grid_images(const char *folder_path, GridLetter **letters) {
    ...
    // Première passe: compter les fichiers valides
    int count = 0;
    const gchar *filename;
    
    while ((filename = g_dir_read_name(dir)) != NULL) {
        int x, y;
        if (parse_grid_filename(filename, &x, &y)) {
            count++;
        }
    }
    ...
    *letters = malloc(count * sizeof(GridLetter));
    ...
    // Deuxième passe: charger les images
    g_dir_rewind(dir);
    int loaded = 0;
...
// Structure pour trier les fichiers par numéro
typedef struct {
    char *filename;
    int number;
} NumberedFile;
...
// comparator for sorting folder name pointers stored in a GPtrArray
...
static gboolean parse_number_filename(const char *filename, int *number) {
    // Format attendu: letter_N.png (ex: letter_0.png, letter_1.png, letter_15.png)
    // ou N.bmp pour rétrocompatibilité
    int len = strlen(filename);
    
    // Vérifier l'extension .png ou .bmp
    ...
    int ext_len = 4; // longueur de ".png" ou ".bmp"
    
    // Vérifier le format "letter_N.png"
    if (len > 11 && strncmp(filename, "letter_", 7) == 0) {
        // Extraire le nombre entre "letter_" et ".png"
        char num_str[16];
        int num_len = len - 7 - ext_len; // longueur du nombre
        
        if (num_len >= 16 || num_len < 1) return FALSE;
        
        strncpy(num_str, filename + 7, num_len);
        num_str[num_len] = '\0';
...
char* load_and_predict_word(const char *word_folder, Network *net) {
    ...
    // Collecter et trier les fichiers par numéro
    ...
    for (int i = 0; i < files_array->len; i++) {
...
int load_and_predict_words(const char *words_folder, Network *net, Word **words) {
    ...
    // Compter les sous-dossiers et les stocker
